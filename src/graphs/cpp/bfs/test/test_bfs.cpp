...
TEST(TestBfs, charVertices) {
  ...
  EXPECT_EQ(bfs(graph, Vertex<char>('A'), Vertex<char>('C')), 20);
  EXPECT_EQ(bfs(graph, Vertex<char>('A'), Vertex<char>('F')), 8);
  EXPECT_EQ(bfs(graph, Vertex<char>('B'), Vertex<char>('G')), 15);
  EXPECT_EQ(bfs(graph, Vertex<char>('E'), Vertex<char>('A')), INT_MAX);
  EXPECT_EQ(bfs(graph, Vertex<char>('C'), Vertex<char>('D')), INT_MAX);
}

TEST(TestBfs, intVertices) {
  Graph<int> graph;
  ...
  EXPECT_EQ(bfs(graph, Vertex<int>(2), Vertex<int>(0)), 2);
  EXPECT_EQ(bfs(graph, Vertex<int>(2), Vertex<int>(1)), 1);
  EXPECT_EQ(bfs(graph, Vertex<int>(2), Vertex<int>(2)), 0);
  EXPECT_EQ(bfs(graph, Vertex<int>(2), Vertex<int>(3)), 1);
  EXPECT_EQ(bfs(graph, Vertex<int>(2), Vertex<int>(4)), 2);
}

TEST(TestBfs, nonExistentVertices) {
  Graph<int> graph;

  EXPECT_EQ(bfs(graph, Vertex<int>(1), Vertex<int>(1)), INT_MAX);
  EXPECT_EQ(bfs(graph, Vertex<int>(1), Vertex<int>(2)), INT_MAX);
  EXPECT_EQ(bfs(graph, Vertex<int>(1), Vertex<int>(3)), INT_MAX);
  EXPECT_EQ(bfs(graph, Vertex<int>(1), Vertex<int>(4)), INT_MAX);
  EXPECT_EQ(bfs(graph, Vertex<int>(1), Vertex<int>(5)), INT_MAX);
}
