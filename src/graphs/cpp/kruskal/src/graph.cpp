...
Vertex::Vertex() : value(0), color(Color::WHITE) {}

Vertex::Vertex(int _value) : value(_value), color(Color::WHITE) {}

Vertex::Vertex(const Vertex &vertex)
    : value(vertex.value), color(vertex.color) {}
...
size_t Vertex::HashFunction::operator()(const Vertex &vertex) const {
  return std::hash<int>()(vertex.value) ^
         (std::hash<int>()(static_cast<int>(vertex.color)) << 1);
}
...
Graph::Graph(std::unordered_map<Vertex, std::vector<Edge>, Vertex::HashFunction>
                 adjcDict)
    : adjcDict(adjcDict) {}
...
unsigned int Graph::size() const { return adjcDict.size(); }

bool Graph::empty() const { return adjcDict.empty(); }
